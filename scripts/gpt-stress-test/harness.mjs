// Stress-test harness for GymTracker GPT food search.
// Extracts the LIVE system prompt from gptService.js so prompt edits are picked up automatically,
// replicates the exact API call + normalization, runs a case file, evaluates, writes results JSON.
//
...
// ---- extract live system prompt (first one = text search) ----
...
async function query(input) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (gptData.error?.message?.includes("Rate limit") && attempt < 8) {
      await sleep(3000 + Math.random() * 4000);
      continue;
    }
    break;
  }
  if (gptData.error) return { status: "api_error", error: gptData.error.message };
  const rawText = gptData.output_text ?? gptData.output?.find(o => o.type === "message")?.content?.[0]?.text;
  if (!rawText) return { status: "app_error", error: "No response from GPT" };
  let parsed;
  try {
    parsed = JSON.parse(rawText.replace(/```json/g, "").replace(/```/g, "").trim());
  } catch {
    return { status: "app_error", error: "Failed to parse GPT JSON output", rawText };
  }
  ...
  if (!Array.isArray(parsed.items)) return { status: "app_error", error: "Invalid GPT output", rawText };
