  ...
  <Modal visible={visible} transparent animationType="fade">
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={styles.modalOverlay}>
        <Pressable style={StyleSheet.absoluteFill} onPress={() => { Keyboard.dismiss(); setVisible(false); }} />
            ...
            {customFoods.map((food) => (
              <View key={food.id} style={styles.foodCard}>
                ...
                <View style={styles.foodActionsRow}>
                  ...
                  <Pressable
                    onPress={() => setCustomFoods((f) => f.filter((x) => x.id !== food.id))}
                    style={({ pressed }) => [styles.customDelete, pressed && styles.customDeletePressed]}
                  >
